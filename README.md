...
智能数据操作平台 — 集 **本体引擎**、**数据集成**、**AI 平台**、**低代码应用** 于一体。面向中小企业和国产化场景。
...
- `scripts/deploy_remote.sh`：同步代码到远程服务器并重建 `backend` / `frontend`
- `scripts/rebuild_remote_demo.sh`：同步代码后远程执行 Demo 重建，并输出核心计数
...
更详细的用法与运维说明见 [DEPLOY.md](DEPLOY.md)。
...
- **Action 写回引擎** — 执行成功后通过 Webhook/REST API 回写外部业务系统 (ERP/CRM)，支持重试 + 模板变量
- **Ontology Functions** — Python 表达式安全沙箱，可计算派生列/校验/Agent 工具调用
- **AI Ontology Builder** — 自然语言描述业务 → AI 自动生成完整对象类型 + 属性 + 关联 + 操作
...
- AI Agent Studio — 5 类 10 个工具:
  ...
  - `aip_functions`: AIP 函数调用
  - `ontology_functions`: 计算函数调用
...
- [DEPLOY.md](DEPLOY.md) — **部署与运维手册**（服务器部署、配置、备份、更新、排障）
- [CASES.md](CASES.md) — OntoForge 能力与案例参考
- [ROADMAP.md](ROADMAP.md) — 开发路线图 (V1.0 Phase 0-6 完成 / V2.0 Phase 7+ 进行中)
